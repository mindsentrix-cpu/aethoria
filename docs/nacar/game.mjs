import{ZONES,SKILLS,CLUES,RECIPES,freshState,parseSave,serialize,rank,skillLevel,playerLevel,maxHP,maxAir,nearest,objective,objectivePosition,TILE,tutorialGuide,tutorialProgress,LESSONS,learnMenu,storyObjective}from'./model.mjs?v=zoom-6';
...
import{screenLayout,playFrame,compactObjective}from'./layout.mjs?v=zoom-6';
import{SAVE,BACKUP,MAX_JOURNEY_SIZE,readJourney,exportJourney,restoreJourney}from'./journey.mjs?v=zoom-6';
import{CameraZoom,bindCameraGestures,MIN_ZOOM,MAX_ZOOM}from'./camera.mjs?v=zoom-6';
...
const cameraZoom=new CameraZoom(game.state.settings.cameraZoom);
let cameraBadgeTimer=null;
const isPageZoomed=()=>Boolean(window.visualViewport&&window.visualViewport.scale>1.01);
const cameraInput=bindCameraGestures({surface,world:canvas,controls:touch,zoom:cameraZoom,isPageZoomed,isPlaying:()=>!game.menu&&!modal,onChange:value=>applyCameraZoom(value,true),onCommit:save});
function applyCameraZoom(value,showHint=false){
 renderer.setCameraZoom(value);game.state.settings.cameraZoom=value;
 const percent=Math.round(value*100),readout=$('camera-readout'),slider=$('camera-range'),badge=$('camera-badge');
 if(readout)readout.textContent=percent+' %';
 if(slider){slider.value=String(percent);slider.setAttribute('aria-valuetext',percent+' por ciento')}
 const less=dialogs.querySelector('[data-action="zoom-out"]'),more=dialogs.querySelector('[data-action="zoom-in"]');
 if(less)less.disabled=value<=MIN_ZOOM;if(more)more.disabled=value>=MAX_ZOOM;
 if(badge){badge.textContent='Cámara · '+percent+' %';if(showHint&&!modal){clearTimeout(cameraBadgeTimer);badge.hidden=false;cameraBadgeTimer=setTimeout(()=>badge.hidden=true,1400)}}
}
function chooseCameraZoom(value){cameraInput.cancel();applyCameraZoom(cameraZoom.set(value),true);save()}
const icon=(kind)=>{const paths={camera:'M3 8V3h5M16 3h5v5M21 16v5h-5M8 21H3v-5M8 12h8M12 8v8',menu:'M4 6h16M4 12h16M4 18h16',skills:'M4 18V12M10 18V7M16 18V3M3 21H21',map:'M3 5l6-2 6 2 6-2v16l-6 2-6-2-6 2zM9 3v16M15 5v16',bag:'M5 7h14v14H5zM8 7V3h8v4M5 13h14M10 12v3h4v-3',journal:'M5 3h14v18H5zM8 8h8M8 12h8M8 16h5',pause:'M8 4v16M16 4v16',sound:'M3 9h4l5-5v16l-5-5H3zM16 7q6 5 0 10'};return`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="${paths[kind]||paths.journal}"/></svg>`};
...
function clearInput(){keys={};stick={x:0,y:0};holdAttack=false;cameraInput.cancel();surface.dataset.playing=String(!game.menu&&!modal);const k=document.querySelector('.knob');if(k)k.style.transform='translate(0px,0px)'}
...
function hud(){
 ...
 <div class="area-label"><strong id="area-name"></strong><span id="area-sub"></span></div><button class="interact-label" data-action="interact" id="interaction" style="display:none"></button><div class="hint">WASD · E interactuar · ESPACIO herramienta · Q esquivar</div><div id="camera-badge" class="camera-badge" aria-hidden="true" hidden></div>`;
 lastObjective='';lastNear='';buildTouch();updateHUD();measureHUD();applyCameraZoom(cameraZoom.value);
...
function begin(){returnToTitle=false;dialogs.innerHTML='';touch.style.display='';modal=null;game.menu=false;clearInput();cameraZoom.set(game.state.settings.cameraZoom);hud();renderer.ready=false;audio('click');save()}
...
function showMenu(name,justCrafted=null){audio('click');const s=game.state,lesson=tutorialGuide(s);let content='';
 if(name==='menu'){
  content=modalHeader(ZONES[s.zone].name,'Tu viaje')+`<div class="menu-grid">${[['skills','Habilidades','Nivel '+playerLevel(s)],['map','Mapa','Sectores y rutas'],['bag','Mochila','Materiales y salud'],['journal','Diario',s.clues.length+' pruebas'],['guide','Guía','Tu siguiente paso'],['camera','Cámara','Acercar, alejar o restablecer'],['pause','Pausa','Sonido y ayuda'],['play-mode','Pantalla completa','Jugar fuera de ChatGPT'],['journey','Tu partida','Guardar o cargar copia']].map(([a,n,d])=>`<button class="secondary" data-action="${a}">${icon(a)}<span><strong>${n}</strong><small>${d}</small></span></button>`).join('')}</div>`;
 }
 else if(name==='guide'){
  const o=objective(s),g=tutorialGuide(s),act=({bag:'bag',journal:'journal',map:'map',skills:'skills',scan:'scan'}[g?.control]);
  content=modalHeader('TU SIGUIENTE PASO',o[0])+`<p>${touchMode()?o[1]:o[1].replace('Mueve el control izquierdo','Usa WASD o las flechas')}</p><p class="small">${touchMode()?'Muévete con el control izquierdo. Usa los botones de la derecha para actuar, golpear y esquivar.':'WASD o flechas: moverte. E: interactuar. Espacio: golpear. Q: esquivar.'}</p><p class="small">Separa dos dedos sobre el escenario para acercar la cámara; júntalos para alejarla. En Menú → Cámara puedes ajustar el zoom o volver al 100 %.</p><div class="row">${act?`<button class="primary" data-action="${act}">${({bag:'Abrir Mochila',journal:'Abrir Diario',map:'Abrir Mapa',skills:'Ver Habilidades',scan:'Activar escáner'})[act]}</button>`:'<button class="primary" data-action="close">Seguir jugando</button>'}</div>`;
 }
 else if(name==='camera'){
  const percent=Math.round(cameraZoom.value*100);
  content=modalHeader('A TU DISTANCIA','Cámara')+`<p>Separa dos dedos sobre el escenario para acercar. Júntalos para alejar. La vista sigue centrada en Noa.</p><label class="field-label" for="camera-range">Distancia de la cámara <output id="camera-readout" for="camera-range">${percent} %</output></label><div class="camera-adjust"><button class="secondary" data-action="zoom-out" aria-label="Alejar cámara" ${cameraZoom.value<=MIN_ZOOM?'disabled':''}>−</button><input id="camera-range" type="range" min="70" max="180" step="1" value="${percent}" aria-valuetext="${percent} por ciento"><button class="secondary" data-action="zoom-in" aria-label="Acercar cámara" ${cameraZoom.value>=MAX_ZOOM?'disabled':''}>+</button></div><button class="secondary camera-reset" data-action="zoom-reset">Restablecer · 100 %</button><p class="small">70 % para ver más entorno · 180 % para ver los detalles.<br>En teclado: + / − para ajustar; 0 para restablecer.</p>`;
 }
...
dialogs.addEventListener('input',e=>{if(e.target.id==='camera-range')applyCameraZoom(cameraZoom.set(Number(e.target.value)/100))});
dialogs.addEventListener('change',e=>{if(e.target.id==='camera-range')save()});
...
function resize(){
 resizeQueued=false;
 const pageZoom=String(isPageZoomed());if(surface.dataset.pageZoom!==pageZoom){surface.dataset.pageZoom=pageZoom;clearInput()}
